...
侧边栏技能库和“技能”会话视图使用同一个目录。每次打开技能库都会先让当前会话的缓存失效再加载，因此安装到用户技能目录（`~/.agents/skills`）的新 skill 不需要重连就会出现。面向用户的目录会排除 DSH 自有的 `dsh-*` 和 bundled 条目；面向模型的 skill 指令不受这个展示过滤影响。技能库还提供仅保存在当前浏览器中的分组、标签和最近使用排序。
