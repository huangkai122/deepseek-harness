...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const api: ApiProxy = {
    db: {
      testConnection: request => ok(request, {
        success: false,
        error: 'Database connection tests are unavailable in the fixture client.',
      }),
    },
    sessions: {
...
export class FixtureApiClient extends AbstractApiClient {
  ...
  private dispatch(
    ...
    switch (method) {
      case 'db.testConnection': return this.api.db.testConnection(request)
