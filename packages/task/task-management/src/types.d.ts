...
export declare function LeaseToken(value: string): LeaseToken;
/** The five columns shown by the task board. */
export type TaskPrimaryStatus = 'pending' | 'clarifying' | 'developing' | 'ready_for_test' | 'released';
/** Execution detail orthogonal to the board column. */
export type TaskExecutionStatus = 'idle' | 'queued' | 'running' | 'waiting_for_user' | 'waiting_workspace_cleanup' | 'waiting_workspace' | 'paused' | 'failed' | 'canceled';
...
export type TaskFailureKind = 'network_error' | 'worker_crash' | 'temporary_database_error' | 'compile_failed' | 'test_failed' | 'git_conflict' | 'dirty_workspace' | 'agent_failed' | 'permission_error';
/** Whether a failure may be retried without human intervention. */
export declare function isAutomaticRetryFailure(kind: TaskFailureKind): boolean;
